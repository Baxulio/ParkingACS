...
manager.connectToHost("192.168.0.210");
QNetworkRequest request(QUrl("http://192.168.0.210/webcapture.jpg?command=snap&channel=1"));

qDebug()<<"start";

t.start();
QObject::connect(&manager, &QNetworkAccessManager::finished, [t](QNetworkReply* pReply){
    qDebug()<<t.elapsed();
    QPixmap pix;
    pix.loadFromData(pReply->readAll());
    qDebug()<<t.elapsed()<<sizeof(pix)<<sizeof(bool);
